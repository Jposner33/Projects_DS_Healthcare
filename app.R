...
server <- function(input, output){
  ...
  output$map_before_aca <- renderPlotly({
    # Dynamically fetch the dataframe for the selected year before the ACA
    ...
    # Dynamically fetch the dataframe for the selected year after the ACA
    ...
    # Compute shared range for consistent legend: combine before_aca and after_aca for the selected variable
    ...
    p_before <- ggplot(data = before_aca) +
      ...
      labs(fill = "%") +  # Ensure the fill label is set
      theme(legend.position = "right")  # Ensure the legend is visible and correctly placed
  ...
  output$map_after_aca <- renderPlotly({
    ...
    # Join with MN counties for post-ACA data
